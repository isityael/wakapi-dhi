...
func (suite *SummaryServiceTestSuite) TestSummaryService_HeartbeatCreateEvent_ClearsNewerSummaries() {
	...
	assert.Eventually(suite.T(), func() bool {
		return mockCalledWith(suite.SummaryRepository.Calls, "DeleteByUserAfter", TestUserId, historicalHeartbeatTime)
...
func mockCalledWith(calls []mock.Call, method string, args ...interface{}) bool {
	for _, call := range calls {
		...
		for i, expected := range args {
			if !assert.ObjectsAreEqual(expected, call.Arguments[i]) {
				matches = false
				break
			}
		}
